...
- 超文本传输安全协议。
  - https 是基于 http协议的，不过https会使用 TLS/SSL 对数据加密。使用 TLS/SSL 协议，所有的信息都是加密的，第三方没办法窃听。并且他提供了一种校验机制，信息一旦被篡改，通信的双方会立刻发现。它还配备了身份证书，防止身份被冒充的情况出现。
  - TLS握手的过程：
    1. 客户端发送协议版本号、支持的加密方法、一个随机数给服务端
    2. 服务端收到后，发送服务端证书、确认加密方法、生成一个随机数给客户端
    3. 客户端确认服务端证书有效后，生成一个新的随机数，并使用服务端证书的公匙对其加密，并且还会提供一个前面所有内容的hash值，发送给服务器，供服务器校验
    4. 服务端使用私匙对服务端发来的随机数进行解密。并提供前面所有内容的hash值来提供客户端检验。
...
- 设置同域下共享的 `localStorage` 与 监听 wondow.onstorage
  - 重复写入相同的值无法触发
  - 会受到浏览器隐身模式等的限制
...
- 回流：当元素的尺寸、结构、或者触发某些属性时，浏览器会重新绘制整个页面，成为回流。此时，浏览器需要重新经过计算，计算后还需要重新页面布局，因此消耗较大。会触发回流的操作有：
  - 页面初次渲染
  - 浏览器窗口大小的改变
  - 元素的尺寸、位置、内容发生变化
  - 元素字体大小的改变
...
- `cookie`：通常用于存储用户身份信息，登录状态等
  - http请求中自动携带，体积上限为 4k，可自行设置过期事件
...
采用分代回收的思想，内存分为 新生代和老生代空间，针对不同的空间采用不同的 GC 算法。设计到的算法有：
  - 分代回收
  - 空间复制
  - 标记清除
  - 标记整理
