...
- 超文本传输安全协议。
  - https 是基于 http 协议的，不过 https 会使用 TLS/SSL 对数据加密。使用 TLS/SSL 协议，所有的信息都是加密的，第三方没办法窃听。并且他提供了一种校验机制，信息一旦被篡改，通信的双方会立刻发现。它还配备了身份证书，防止身份被冒充的情况出现。
  - TLS握手的过程：
...
采用分代回收的思想，内存分为 新生代和老生代 空间，针对不同的空间采用不同的 GC 算法。设计到的算法有：
  - 分代回收
  - 空间复制
  - 标记清除
  - 标记整理
